...
def check_sig(sig, contents):
	...
	key = input("> ")
	if(test_key(key, sig, contents, headDict)):
		if len(key) > 25:
...
def test_key(key, sig, contents, headDict):
	confirmed = False
...
def sign_token(headDict, paylDict, key, keyLength):
	newHead = headDict
...
def check_pubkey_bypass(headDict, tok2):
	...
	try:
		with open(pubKey) as filekey:
			...
			newHead = build_head("HS256",headDict)
			newTok = newHead+"."+tok2
...
def tamper_token(paylDict, headDict):
	...
	print("[4] Sign token with key file  (symmetric)")
	...
	if selection == 1:
		...
		if selLength == 2:
			...
			keyLength = 256
		newSig, badSig, newContents = sign_token(headDict, paylDict, key, keyLength)
		...
		jsonDump = json.dumps(paylDict,separators=(",",":"))
		tok2 = (base64.urlsafe_b64encode(jsonDump.encode('utf-8'))).decode('utf-8').strip("=")
		check_pubkey_bypass(headDict, tok2)
		exit(1)
	if selection == 4:
